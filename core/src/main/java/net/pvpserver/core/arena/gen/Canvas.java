...
final class Canvas {
    ...
    /** Yaw facing +z. */
    static final float SOUTH = 0f;
    /** Yaw facing -x. */
    static final float WEST = 90f;
    /** Yaw facing -z. */
    static final float NORTH = 180f;
    /** Yaw facing +x. */
    static final float EAST = -90f;

    final TemplateBuilder b;
    final Random rnd;
    final Noise noise;

    Canvas(int sizeX, int sizeY, int sizeZ, long seed) {
    ...
    void setIfAir(int x, int y, int z, String block) {
    ...
    static String facingAway(int x, int z, double cx, double cz) {
        double dx = x + 0.5 - cx;
        double dz = z + 0.5 - cz;
    ...
    static String facingToward(int x, int z, double cx, double cz) {
    ...
    void disc(double cx, int y, double cz, double r, String block) {
    ...
    void ring(double cx, int y, double cz, double rInner, double rOuter, String block) {
    ...
    void perimeter(int x1, int z1, int x2, int z2, int y1, int y2, String block) {
        for (int y = y1; y <= y2; y++) {
            for (int x = x1; x <= x2; x++) {
                set(x, y, z1, block);
                set(x, y, z2, block);
    ...
    /** Vertical column. */
    void column(int x, int z, int y1, int y2, String block) {
        for (int y = y1; y <= y2; y++) {
            set(x, y, z, block);
        }
    }
    ...
    void underside(double cx, int topY, double cz, double radius, int depth, String... blocks) {
    ...
    void ball(double cx, double cy, double cz, double r, boolean onlyAir, String... blocks) {
    ...
    void lanternPost(int x, int y, int z, String post, int height, String lantern) {
    ...
    void broadTree(int x, int y, int z, int height, String log, String leaves) {
    ...
    void conifer(int x, int y, int z, int height, String log, String leaves) {
        String leaf = leaves + "[persistent=true]";
        int top = y + height;
        set(x, top, z, leaf);
        set(x, top - 1, z, leaf);
    ...
    void plant(int x, int y, int z, String plant, String... grounds) {
    ...
    void pad(int cx, int y, int cz, int radius, String top, String under) {
